...
private:
	void TranslationUnit(const char*);

	// Emit Process
	void BeginProgram(const char*);
	void Data_Segment();
	void EmitGlobals();
	void Text_Segment();
	void EmitFunctions();
	...
	// About IO stream
	void SetUpASMfile(const char *output);
...
private:
	...
	void emit(const char*,const char *output);

public:
	void Compilation(const char *input,const char *output = NULL);
};
